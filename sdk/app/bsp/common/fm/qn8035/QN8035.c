...
//测试频率：24M
#define LOG_TAG_CONST       NORM //QN8035
#define LOG_TAG             "[QN8035]"
#include "log.h"

#include "fm_iic_dri.h"
#define app_IIC_write(a, b, c, d) \
	  fm_iic_write(a, b, c, d)
/* #define app_IIC_readn(a, b, c, d)  \ */
/* fm_iic_readn(a, b, c, d) */
...
/*     u8  byte; */
/*     #<{(| g_iic_busy = 1; |)}># */
/*     iic_start(iic_dev);                    //I2C启动 */
/*     iic_tx_byte(iic_dev, 0x20);             //写命令 */
/*     iic_tx_byte(iic_dev, addr);         //写地址 */
/*     iic_start(iic_dev);                    //写转为读命令，需要再次启动I2C */
/*     iic_tx_byte(iic_dev, 0x21);             //读命令 */
/*     iic_stop(iic_dev);                     //I2C停止 */
/*     byte = iic_rx_byte(iic_dev, 0); */
/*     #<{(| g_iic_busy = 0; |)}># */
/*     return  byte; */
/* } */
...
bool qn8035_set_fre(u16 fre)
{
    if (QND_RXValidCH(fre)) {
        QND_TuneToCH(fre);
        log_info("qn8035 set fre(%d) ok!", fre);
        return TRUE;
    }
    log_info("qn8035 set fre fail!");
    return FALSE;
}
...
bool qn8035_read_id(void)
{
    ...
    if (0x84 == cChipID) {
        qn8035_online = 1;
        log_info("qn8035 read id ok!id:%x", cChipID);
        return 1;
    } else {
        qn8035_online = 0;
        log_info("qn8035 read id fail!");
        return 0;
    }
}
...
void qn8035_test()
{
    ...
    if (qn8035_read_id()) {
        qn8035_init();
        qn8035_set_fre(875);//(875);
        /* QND_WriteReg(0x14, 0x47); */
        /* QND_SetVol(100); */
    }
}
#endif

...
static u32 cdc_setup_rx(struct usb_device_t *usb_device, struct usb_ctrlrequest *ctrl_req)
{
    ...
    u8 *ep0_buffer = usb_get_ep_buffer(usb_id, 0);
    memcpy(read_ep, ep0_buffer, len);
...
u32 cdc_desc_config(const usb_dev usb_id, u8 *ptr, u32 *itf)
{
    ...
    tptr[8 + 9 + 5 + 5 + 4 + 5 + 2] = *itf + 1;
    //bulk out ep
    tptr[8 + 9 + 5 + 5 + 4 + 5 + 9 + 2] = CDC_DATA_EP_OUT;
    tptr[8 + 9 + 5 + 5 + 4 + 5 + 9 + 4] = MAXP_SIZE_CDC_BULKOUT & 0xff;
    tptr[8 + 9 + 5 + 5 + 4 + 5 + 9 + 5] = (MAXP_SIZE_CDC_BULKOUT >> 8) & 0xff;
    //bulk in ep
    tptr[8 + 9 + 5 + 5 + 4 + 5 + 9 + 7 + 2] = USB_DIR_IN | CDC_DATA_EP_IN;
    tptr[8 + 9 + 5 + 5 + 4 + 5 + 9 + 7 + 4] = MAXP_SIZE_CDC_BULKIN & 0xff;
    tptr[8 + 9 + 5 + 5 + 4 + 5 + 9 + 7 + 5] = (MAXP_SIZE_CDC_BULKIN >> 8) & 0xff;
...
static void cdc_intrrx(struct usb_device_t *usb_device, u32 ep)
{
    ...
    u32 len = usb_g_ep_read(usb_id, CDC_DATA_EP_OUT, cdc_rx_buf, MAXP_SIZE_CDC_BULKOUT, 0);
    if (cdc_hdl->output) {
...
u32 cdc_read_data(const usb_dev usb_id, u8 *buf, u32 len)
{
    ...
    //由于bulk传输使用双缓冲，无法用usb_get_ep_buffer()知道是哪一个buffer，需要外部buffer接收数据
    rxlen = usb_g_ep_read(usb_id, CDC_DATA_EP_OUT, cdc_rx_buf, MAXP_SIZE_CDC_BULKOUT, 0);
...
u32 cdc_write_data(const usb_dev usb_id, u8 *buf, u32 len)
{
    ...
    while (offset < len) {
        ...
        txlen = usb_g_ep_write(usb_id, CDC_DATA_EP_IN, buf + offset, txlen);
        if (txlen == 0) {
            break;
        }
        if ((cdc_hdl->bmTransceiver & (BIT(1) | BIT(4))) != (BIT(1) | BIT(4))) {
    ...
    if ((offset % MAXP_SIZE_CDC_BULKIN) == 0) {
        usb_g_ep_write(usb_id, CDC_DATA_EP_IN, NULL, 0);
    }
    /* os_mutex_post(&cdc_hdl->mutex_data); */
    return offset;
}
...
static u8 cdc_in_buf[64] __attribute__((aligned(32)));
static u8 cdc_out_buf[64] __attribute__((aligned(32)));
void cdc_register(const usb_dev usb_id)
{
    ...
    if (!cdc_hdl) {
        ...
        cdc_hdl->bulk_ep_in_buffer = usb_get_ep_buffer(usb_id, CDC_DATA_EP_IN | USB_DIR_IN);
        cdc_hdl->bulk_ep_out_buffer = usb_get_ep_buffer(usb_id, CDC_DATA_EP_OUT);

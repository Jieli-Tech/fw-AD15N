...
#define SPI2_ENABLE   1   //是否使能SPI1，使能后需定义spi2_p_data
...
static void spi1_iomc_config(spi_dev spi, u8 mode)
{
    ...
    if (mode == SPI_MODE_UNIDIR_4BIT) {
        ...
        gpio_set_fun_input_port(port[3], PFI_SPI1_DA2);
        gpio_set_fun_input_port(port[4], PFI_SPI1_DA3);
    }
}
...
{
    u8 *port = spi_get_info_port(spi);
    gpio_set_fun_output_port(port[0], FO_SPI2_CLK, 1, 1);
    gpio_set_fun_output_port(port[1], FO_SPI2_DA0, 1, 1);
    gpio_set_fun_input_port(port[0], PFI_SPI2_CLK);
    gpio_set_fun_input_port(port[1], PFI_SPI2_DA0);
    if (mode != SPI_MODE_UNIDIR_1BIT) {
        gpio_set_fun_output_port(port[2], FO_SPI2_DA1, 1, 1);
        gpio_set_fun_input_port(port[2], PFI_SPI2_DA1);
    }
//spi2 no 4bit
}
...
int spi_open(spi_dev spi)
{
    ...
    spi_w_reg_con(spi_regs[id], BIT(14));
...
void spi_close(spi_dev spi)
{
    ...
    if (spi == SPI1) { //spi1
        gpio_disable_fun_output_port(port[0]);
        gpio_disable_fun_output_port(port[1]);
        gpio_disable_fun_output_port(port[2]);
        gpio_disable_fun_output_port(port[3]);
        gpio_disable_fun_output_port(port[4]);

...
#define INPUT_CHANNLE1_SRC_SEL(x)		SFR(JL_IOMC->IOMC2, 5, 5, x)
#define IRFLT_OUTPUT_TIMER_SEL(x)		SFR(JL_IOMC->IOMC1, 3, 1, x)
#define CAP_TIMER_SEL(x)	        	SFR(JL_IOMC->IOMC1, 4, 2, x)

#if 0

/* #define IRTMR       JL_TMR0 */
/* #define IRQ_IRTMR   IRQ_TIME0_IDX */
/* #define IOMC_CAP    (1<<4) */
...
void set_ir_clk(void)
{
    ...
    IRTMR->PRD = 0;
    IRTMR->CNT = 0;
    IRTMR->CON = ((3 << 4) | (2 << 2) | (3 << 0));  //htc + falling edge
}

...
static u8 ut_tx_pin[] = {
    IO_PORTA_04, IO_PORTA_00, IO_PORTA_04, IO_PORTA_00, //UT0_TX
    IO_PORT_DM, IO_PORTA_09, IO_PORT_DM, IO_PORTA_09,  //UT1_TX
};
#define ICH0SEL 40//////////////////////////////////////////////////////
#define ICH2SEL 42//////////////////////////////////////////////////////
static u8 ut_rx_pin[] = {
    IO_PORTA_05, IO_PORTA_01, ICH2SEL, ICH2SEL, //UT0_RX
    IO_PORT_DP, IO_PORTA_09, ICH0SEL, ICH0SEL,  //UT1_RX
};
...
#define USB_DP_OUT(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(0)) | ((x & 0x1)<<0))
#define USB_DM_OUT(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(1)) | ((x & 0x1)<<1))
#define USB_DP_DIR(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(2)) | ((x & 0x1)<<2))
#define USB_DM_DIR(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(3)) | ((x & 0x1)<<3))
#define USB_DP_PD(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(4)) | ((x & 0x1)<<4))
#define USB_DM_PD(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(5)) | ((x & 0x1)<<5))
#define USB_DP_PU(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(6)) | ((x & 0x1)<<6))
#define USB_DM_PU(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(7)) | ((x & 0x1)<<7))
#define USB_DP_DIE(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(9)) | ((x & 0x1)<<9))
#define USB_DM_DIE(x)    JL_IOMC->USB_IO_CON0 = ((JL_IOMC->USB_IO_CON0 & ~BIT(10)) | ((x & 0x1)<<10))
...
//配置输入通道
    ...
    if (rx_pin == IO_PORT_DP) {
        USB_DP_IN();
        USB_DP_PU(1);
        return 1;
    } else if (rx_pin == IO_PORT_DM) {
        USB_DM_DIR(1);
        USB_DM_PU(1);
        return 1;
    } else {
...
u8 gpio_set_uart1(u32 ut_ch)  //ch=-1；0；1；2；3
{
    ...
    if (ut_ch == 0) {
        ...
        gpio_set_direction(IO_PORTA_09, 1);//in  /*JL_PORTA->DIR |= BIT(5);*/
        gpio_set_direction(IO_PORTA_09, 0);//out /*JL_PORTA->DIR &= ~BIT(4);*/
...
void uart01_test()
{
    ...
    arg.tx_pin = IO_PORTA_06; //IO_PORT_DM; //IO_PORTA_02;
    arg.rx_pin = IO_PORTA_07; //IO_PORT_DP;

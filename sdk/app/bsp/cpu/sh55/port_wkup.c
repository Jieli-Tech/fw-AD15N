...
static void port_wkup_isr()
{
    if ((JL_WAKEUP->CON0 & BIT(0)) && (JL_WAKEUP->CON3 & BIT(0))) {
        ...
        log_info("port %d wakeup", (JL_IOMC->IOMC2 >> 12) & 0x3f);
    ...
    if ((JL_WAKEUP->CON0 & BIT(1)) && (JL_WAKEUP->CON3 & BIT(1))) {
        ...
        log_info("IRFLT %d wakeup", (JL_IOMC->IOMC2 >> 6) & 0x3f);
...
{
    u8 i = 0;
    u8 sd0_dat_io[4] = {IO_PORTA_03, IO_PORTB_02, IO_PORTB_06, IO_PORTA_08};
...
void port_wkup_test_main()
{
    ...
    port_wkup_enable(IO_PORTA_08, 1, port_wkup_irq_cbfun_test); //PA08-IO中断，1:下降沿触发，回调函数port_wkup_irq_cbfun_test
    /* port_wkup_disable(IO_PORTA_08); */
    uartx_wkup_enable(0, IO_PORTA_07, 1, uart0_rx_wkup_irq_cbfun_test); //uart0 rx PA7-IO中断，1:下降沿触发，回调函数uart0_rx_wkup_irq_cbfun_test
    /* uartx_wkup_disable(0,IO_PORTA_07); */
    uartx_wkup_enable(1, IO_PORTB_09, 1, NULL); //uart1 rx PB9-IO中断，1:下降沿触发，回调函数:NULL无
    /* uartx_wkup_disable(1,IO_PORTB_09); */
    irflt_wkup_enable(IO_PORTA_09, 0, irflt_wkup_irq_cbfun_test); //PA9-IO中断，0:上升沿触发，回调函数irflt_wkup_irq_cbfun_test
    ...
    log_info("iomc0:%x,iomc1:%x", JL_IOMC->IOMC0, JL_IOMC->IOMC1);
}
#endif

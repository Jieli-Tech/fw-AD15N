...
#define IRFF00_MUSIC_DOUBLE_KICK	\
                                /*00*/    NO_MSG,\
                                /*01*/    NO_MSG,\
								/*02*/    NO_MSG,\
								/*03*/    NO_MSG,\
...
const u16 irkey_msg_music_table[][IR_KEY_MAX_NUM] = {			//Music模式下的遥控转换表
    /*短按*/	    {IRFF00_MUSIC_SHORT},
    /*短按抬起*/	{IRFF00_MUSIC_SHORT_UP},
    /*长按*/		{IRFF00_MUSIC_LONG},
    /*连按*/		{IRFF00_MUSIC_HOLD},
    /*长按抬起*/	{IRFF00_MUSIC_LONG_UP},
    /*双击*/		{IRFF00_MUSIC_DOUBLE_KICK},
};
#endif
...
/**@brief   音乐模式按键消息转换函数
   ...
   @param   key_type    按键类型
   @return  无
   @note     static u16 music_msg_filter(u8 key_status, u8 key_num, u8 key_type)
*/
/*----------------------------------------------------------------------------*/
u16 music_msg_filter(u8 key_status, u8 key_num, u8 key_type)
{
    u16 msg = NO_MSG;
    switch (key_type) {
#if KEY_IO_EN
...
#if KEY_IR_EN
    case KEY_TYPE_IR:
        msg = irkey_msg_music_table[key_status][key_num];
        break;
#endif
#if KEY_AD_EN
    case KEY_TYPE_AD:
